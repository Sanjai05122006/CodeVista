...
const CONTACT_RECIPIENT_EMAIL = env.CONTACT_TO_EMAIL?.trim() || "delivered@resend.dev";
...
export const sendContactSubmissionMessage = async (
  ...
  const recipient = CONTACT_RECIPIENT_EMAIL;
  ...
  try {
    ...
    const providerMessage = normalizeProviderMessage(attempt.response, attempt.data);
